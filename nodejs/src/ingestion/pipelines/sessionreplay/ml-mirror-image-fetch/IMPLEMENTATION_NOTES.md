...
| A dominant origin leaves too little diversity to use pod request capacity.                             | Low-origin-diversity pass count, origins and canonical URL jobs at mode entry, and `ml_image_fetch_republished_total{reason="low_origin_diversity",topic="frontier"}`.                                                                                                          | Fetch dashboard: batch diversity, low-origin-diversity deferrals, request capacity, and Kafka lag.                                                                                                      | Tune the origin and tail thresholds only after comparing them with in-flight use and lag. The fixed progress allowance must remain positive.                        |
