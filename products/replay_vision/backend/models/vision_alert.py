...
class VisionAlertConfiguration(TeamScopedRootMixin, UUIDModel):
    ...
    selection = models.JSONField(
        default=dict,
        help_text=(
            "Observation filter: verdict (list[str]), tags (list[str]), min_score/max_score (float), "
            "statuses (list[str], default ['succeeded']). Predicate keys require succeeded-only statuses "
            "because failed observations carry no scanner_result."
        ),
    )
    ...
    class Meta:
        constraints = [
            ...
            models.CheckConstraint(
                condition=models.Q(kind=VisionAlertKind.MATCH) | models.Q(threshold__isnull=False),
