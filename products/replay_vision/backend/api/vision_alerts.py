...
class VisionAlertSelectionSerializer(serializers.Serializer):
    ...
    tags = serializers.ListField(
        child=serializers.CharField(),
        required=False,
        help_text="Classifier tags to match; an observation matches when it carries any of them. Requires succeeded-only statuses.",
    )
    min_score = serializers.FloatField(
        required=False,
        help_text="Minimum scorer score (inclusive). Requires succeeded-only statuses.",
    )
    max_score = serializers.FloatField(
        required=False,
        help_text="Maximum scorer score (inclusive). Requires succeeded-only statuses.",
    )
    statuses = serializers.ListField(
        child=serializers.ChoiceField(choices=[(s.value, s.label) for s in _SELECTABLE_STATUSES]),
        required=False,
        help_text="Observation statuses to include. Defaults to ['succeeded']. Use ['failed'] to alert on failing observations.",
    )
    ...
    def validate(self, attrs: dict) -> dict:
        ...
        if has_predicate and statuses != [ObservationStatus.SUCCEEDED.value]:
            raise ValidationError(
                ...
                "observations have. Remove those filters or set statuses to ['succeeded']."
            )
        return attrs
    ...
    def create(self, validated_data: dict) -> VisionAlertConfiguration:
        ...
        with transaction.atomic():
            ...
            if validated_data["team_id"] not in UNCAPPED_ALERT_TEAM_IDS:
                count = VisionAlertConfiguration.objects.filter(team_id=validated_data["team_id"]).count()
                if count >= MAX_ALERTS_PER_TEAM:
                    raise ValidationError(f"Maximum number of alerts ({MAX_ALERTS_PER_TEAM}) reached for this team.")
            if validated_data.get("kind") == VisionAlertKind.MATCH:
                # The drain, not the scheduler, drives match alerts.
                validated_data.pop("next_check_at", None)
            elif schedule_restriction := validated_data.get("schedule_restriction"):
                validated_data["next_check_at"] = next_allowed_check_at(
                    datetime.now(UTC),
