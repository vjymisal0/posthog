...
class TestVisionAlertMatchOutbox(BaseTest):
    ...
    def test_succeeded_observation_inserts_one_row_per_matching_alert(self) -> None:
        ...
        self._make_match_alert(name="failed only", selection={"statuses": ["failed"]})
    ...
    def test_failed_observation_matches_status_selection_only(self) -> None:
        self._make_match_alert(name="predicate alert")  # verdict predicate: cannot match failures
        failure_alert = self._make_match_alert(name="failure alert", selection={"statuses": ["failed"]})
        ...
        rows = self._outbox_rows()
        assert [row.alert_id for row in rows] == [failure_alert.id]

    def test_ineligible_observation_inserts_nothing(self) -> None:
        self._make_match_alert(selection={"statuses": ["failed"]})
        observation = self._make_pending_observation()
        mark_observation_ineligible_activity(
            MarkObservationIneligibleInputs(
