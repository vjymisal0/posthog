...
export function ScannerAlertsTab({ scannerId, scannerName }: { scannerId: string; scannerName: string }): JSX.Element {
    return (
        <BindLogic logic={scannerAlertsLogic} props={{ scannerId }}>
            <ScannerAlertsTabContent scannerId={scannerId} scannerName={scannerName} />
        </BindLogic>
    )
}

function ScannerAlertsTabContent({ scannerId, scannerName }: { scannerId: string; scannerName: string }): JSX.Element {
    ...
    return (
        <div className="flex flex-col gap-2">
            ...
            <LemonTable
                ...
                emptyState={`No alerts on ${scannerName} yet. Create one to get notified about matching observations or metric thresholds.`}
            />
