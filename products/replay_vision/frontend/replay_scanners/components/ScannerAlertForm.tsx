    ...
    return (
        ...
        >
            <div className="space-y-3">
                <LemonField name="watch" label="Watch">
                    {({ value, onChange }) => (
                        <LemonSegmentedButton
                            value={value}
                            onChange={onChange}
                            options={[
                                { value: 'results', label: 'Scan results' },
                                {
                                    value: 'failures',
                                    label: 'Failed scans',
                                    tooltip: 'Match observations whose scan failed, e.g. to catch a broken scanner.',
                                },
                            ]}
                            size="small"
                        />
                    ...
                    <>
                        ...
                        {scannerType === 'classifier' ? (
                            <LemonField
                                name="tags"
                                label="Tags"
                                info="Matches observations carrying any of these tags."
                            >
                                {({ value, onChange }) => (
                                    <LemonInputSelect
                                        mode="multiple"
...
export function ScannerAlertTrigger(): JSX.Element {
    const { alertForm } = useValues(scannerAlertFormLogic)
        ...
        return (
            <AlertEditorSection title="When it fires">
                <p className="text-muted mb-0">
                    You get one bundled notification per minute covering every new matching observation, so a burst of
                    matches sends one message instead of many.
                </p>
            </AlertEditorSection>
        )
    }
    ...
    return (
        <AlertEditorSection title="When it fires">
            <div className="space-y-4">
                <AlertDefinitionRow label="Alert if">
                    <LemonField name="metric">
                        {({ value, onChange }) => (
                            <LemonSelect
                                ...
                                onChange={onChange}
                                options={[
                                    { value: 'count', label: 'Count of matching observations' },
                    ...
                    <LemonField name="direction">
                        {({ value, onChange }) => (
                            <LemonSelect
                                ...
                                onChange={onChange}
                                options={[
                                    { value: 'above', label: 'is at or above' },
